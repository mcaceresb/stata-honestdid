...
# xx Ok, so the only thing that seems hard here is the CVXR
# xx FLCI_Results <- .findOptimalFLCI_helper
# xx FLCI_Results <- results
...
# xx [this step was fast]
h0 <- .findHForMinimumBias(sigma          = sigma,
                           numPrePeriods  = numPrePeriods,
                           numPostPeriods = numPostPeriods,
                           l_vec          = l_vec)

# xx [this step was fast]
hMin <- .findLowestH(sigma          = sigma,
                     numPrePeriods  = numPrePeriods,
                     numPostPeriods = numPostPeriods,
                     l_vec          = l_vec)

# xx [this step is the one that lags because it goes through a grid for h]
hGrid  <- seq(from = hMin, to = c(h0), length.out = numPoints)
biasDF <- purrr::map_dfr(.x = hGrid,
                         .f = function(h) {
                             .findWorstCaseBiasGivenH(h,
                                                      sigma          = sigma,
                                                      numPrePeriods  = numPrePeriods,
                                                      numPostPeriods = numPostPeriods,
                                                      l_vec          = l_vec,
                                                      returnDF       = T) %>% mutate(h = h)
                         } )

# xx [this step was fast]
biasDF <- biasDF %>% rename(bias = value)
biasDF <-
    left_join(biasDF %>% mutate(id = 1),
              data.frame(m = M, id = 1),
...
results <- list(optimalVec = c(unlist(optimalCIDF$optimal.l), l_vec),
...
# xx .qfoldednormal
# xx .findHForMinimumBias
# xx .findLowestH
# xx .findWorstCaseBiasGivenH
...
.findWorstCaseBiasGivenH <- function(h, sigma, numPrePeriods, numPostPeriods, l_vec, M = 1, returnDF = F) {
    ...
    objectiveBias <- .createObjectiveObjectForBias(numPrePeriods  = numPrePeriods,
                                                   UstackW        = UstackW,
                                                   numPostPeriods = numPostPeriods,
                                                   l_vec          = l_vec)
